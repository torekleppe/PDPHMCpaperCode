...
mdl <- build("realVol.cpp",lambda="lambda_constant",massMatrix = "diagMassVARI")
 
fit.p.c <- run(mdl,Tmax=Tmax,data=list(yy=yy,yinv=yinv,ldets=ldets),
...
save.image(file="Computations_b1")

fit.p.cl <- run(mdl,Tmax=Tmax,data=list(yy=yy,yinv=yinv,ldets=ldets),
               pars=c("h","lnu","lsigmaSq","lphi","mu"),chains=chains,
               #fixedMiDiag=list(zz=1.0),
               init=list(lnu=lnu.0,lphi=lphi.0,mu=mu.0,h=h.0,lsigmaSq=lsigmaSq.0),
